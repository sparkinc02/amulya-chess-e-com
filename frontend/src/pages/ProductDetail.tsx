...
import { ShoppingBag, ArrowLeft, Truck, Shield, RotateCcw, ChevronLeft, ChevronRight, AlertTriangle, Loader2 } from 'lucide-react';
...
export default function ProductDetail() {
  ...
  const { data: productResponse, isLoading, isError } = useGetProductById(id || "");
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-4">
        ...
        <p className="font-mono text-xs uppercase tracking-widest text-muted-foreground">Fetching masterpiece...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="pt-32 text-center px-6">
          ...
          <p className="font-body text-muted-foreground mb-6">The item you're looking for is unavailable.</p>
  ...
  const handleAdd = () => {
    ...
    }, qty); // Pass the local state quantity
    ...
    <div className="min-h-screen bg-background text-foreground">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
              <div className="relative aspect-[4/3] overflow-hidden bg-card border border-border shadow-sm">
                ...
                {/* Badges */}
                {product.isFeatured && (
                  <span className="absolute top-4 left-4 z-20 font-mono text-xs uppercase tracking-wider px-4 py-1.5 bg-bordeaux text-cream">FEATURED</span>
                )}
                {discount && (
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="flex flex-col">
              ...
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-1">
                  <span className={`w-2 h-2 ${product.stock > 0 ? 'bg-green-500' : 'bg-destructive'}`} />
                  <span className="font-mono text-xs uppercase tracking-wider">{product.stock > 0 ? 'In Stock' : 'Out of Stock'}</span>
                </div>
                {product.stock > 0 && product.stock <= 5 ? (
                  <p className="font-mono text-[11px] tracking-wider text-primary flex items-center gap-1.5">
                    <AlertTriangle size={12} /> Only {product.stock} units left — order soon
                  </p>
                ) : product.stock === 0 ? (
                  <p className="font-mono text-[11px] tracking-wider text-destructive">Currently Out of Stock</p>
                ) : (
                  <p className="font-mono text-[11px] tracking-wider text-muted-foreground">Units Available: {product.stock}</p>
                )}
              </div>
              ...
              <div className="flex items-baseline gap-3 mb-8">
                ...
                {product.originalPrice > product.price && (
                  <span className="font-body text-lg text-muted-foreground line-through decoration-primary/30">₹{product.originalPrice.toLocaleString('en-IN')}</span>
                )}
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 mb-8">
                <div className="grid grid-cols-3 border border-border bg-card w-full sm:w-40 h-12 overflow-hidden shadow-sm">
                  <button onClick={() => setQty(Math.max(1, qty - 1))} className="flex items-center justify-center font-mono text-lg hover:bg-muted transition-colors border-r border-border">−</button>
                  <span className="flex items-center justify-center font-mono text-sm">{qty}</span>
                  <button 
                ...
                >
                  <ShoppingBag size={18} className="shrink-0 group-hover:-translate-y-0.5 transition-transform" />
                ...
                <div className="space-y-6 mb-8 border-t border-border pt-8">
                  {product.colors?.length > 0 && (
                    <div className="flex flex-col gap-3">
                      <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Available Colors</span>
                      <div className="flex flex-wrap gap-2">
                        {product.colors.map(c => (
                          <span key={c} className="px-3 py-1.5 border border-border font-mono text-[11px] tracking-tight">{c}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  {product.sizes?.length > 0 && (
                    <div className="flex flex-col gap-3">
                      <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Sizes</span>
                      <div className="flex flex-wrap gap-2">
                        {product.sizes.map(s => (
                          <span key={s} className="px-3 py-1.5 border border-border font-mono text-[11px] tracking-tight">{s}</span>
                        ))}
                      </div>
                    </div>
                  )}
              ...
              <div className="grid grid-cols-3 gap-4 border-t border-border pt-8">
                <div className="flex flex-col items-center text-center gap-2">
                  ...
                  <p className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Free shipping over ₹5000</p>
                </div>
                <div className="flex flex-col items-center text-center gap-2">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-16 border-t border-border pt-12">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={spec.label} className="flex justify-between py-2 border-b border-border/50">
                      <span className="font-mono text-xs uppercase tracking-wider text-muted-foreground">{spec.label}</span>
                      <span className="font-body text-foreground">{spec.value}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {activeTab === 'shipping' && (
              <div className="max-w-2xl space-y-4 font-body text-muted-foreground">
                <p>Free shipping on orders above ₹5,000. Standard delivery within 3-5 business days.</p>
                <p>Easy 7-day returns from date of delivery. Item must be unused, in original packaging.</p>
                <p>Initiate returns via WhatsApp or email with order ID and photos.</p>
              </div>
            )}
          </motion.div>
        </div>
      ...
      {related.length > 0 && (
        <section className="py-16 px-6 bg-card border-t border-border">
          <div className="max-w-7xl mx-auto">
            <h2 className="font-heading text-2xl font-bold text-card-foreground mb-8 uppercase tracking-widest">You May Also Like</h2>
