...
export default function OrderDetail() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {/* Status Banner */}
      <div className={`pt-20 ${statusBannerStyles[order.status] || 'bg-muted border-b-2 border-border'}`}>
          ...
          <div className="mb-10">
            ...
            <div className="flex items-start relative pb-4">
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center text-center relative z-10">
                    <motion.div 
                      ...
                      initial={isDone ? { scale: 0, rotate: -180, opacity: 0 } : { scale: 0, opacity: 0 }}
                      animate={isDone ? { scale: 1, rotate: 0, opacity: 1 } : { scale: 1, opacity: 1 }}
                      transition={{ delay: animDelay, type: "spring", stiffness: 260, damping: 20 }}
                    >
                      {isDone ? <Check size={14} strokeWidth={3} /> : <span className="font-mono text-[10px]">{i + 1}</span>}
                    </motion.div>
                    ...
                    >
                      <p className={`font-mono text-[10px] uppercase tracking-wider font-bold ${isDone ? 'text-foreground' : 'text-muted-foreground'}`}>{step.label}</p>
                      {step.label === 'Order Placed' && <p className="font-mono text-[9px] text-muted-foreground mt-1">{new Date(order.createdAt).toLocaleDateString()}</p>}
                      {step.label === 'Delivered' && isDone && order.updatedAt && <p className="font-mono text-[9px] text-muted-foreground mt-1">{new Date(order.updatedAt).toLocaleDateString()}</p>}
                    </motion.div>
                      ...
                      <div className="absolute top-4 left-[50%] w-full h-[2px] -z-10 bg-muted overflow-hidden">
                         {step.statusMatch.includes(order.status) && (
                           <motion.div
                             ...
                             initial={{ width: "0%" }}
                             animate={{ width: "100%" }}
                             transition={{ delay: animDelay + 0.35, duration: 0.45, ease: "easeInOut" }}
                           />
                         )}
                      </div>
                    )}
              ...
              {order.status === 'CANCELLED' && (
                ...
                >
                  <p className="font-heading font-bold text-destructive uppercase tracking-widest bg-background px-4 py-2 border border-destructive shadow-lg">Order Cancelled</p>
                </motion.div>
              )}
            </div>
          </div>
          ...
          <div className="mb-10">
            ...
            <div className="space-y-3">
              {order.orderItems.map((item, idx) => (
                <div key={idx} className="flex items-center gap-4 p-4 border border-border bg-card">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Size: {item.size} · Color: {item.color}</p>
                    <p className="font-mono text-xs text-muted-foreground mt-1">Qty: {item.quantity} × ₹{item.price.toLocaleString('en-IN')}</p>
                  </div>
                  <span className="font-heading font-bold shrink-0">₹{(item.price * item.quantity).toLocaleString('en-IN')}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div>
              ...
              <div className="border border-border bg-card p-5 space-y-3">
                ...
                <div className="border-t border-border pt-3 flex justify-between">
                  ...
                  <span className="font-heading text-xl font-bold text-primary">₹{order.amount.toLocaleString('en-IN')}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="border border-border bg-card p-5 space-y-4 h-[calc(100%-48px)]">
                <div>
                  ...
                  <p className="font-body">{order.shippingAddress.addressLine}, {order.shippingAddress.apartment && `${order.shippingAddress.apartment}, `} {order.shippingAddress.city}, {order.shippingAddress.state} – {order.shippingAddress.pincode}</p>
                </div>
                <div>
                  <p className="font-mono text-[10px] uppercase tracking-wider text-primary mb-1">Contact Phone</p>
                  <p className="font-body">{order.shippingAddress.phone || user.phone}</p>
                </div>
                <div>
